...
- [x] **Version number in title.** Stripped from main.tex, main.tex, and README.md. SemVerDoc section removed from README. (paper)
...
- [x] **Dead definitions/theorems.** Remaining definitions (`BranchingOracle`, `IsMaximalTrace`, `IsBranchPoint`, `IsDeadEnd`) kept for paper correspondence with docstrings. `IsBranchPoint` is used in CoRefinementConvergence.lean. All other dead code was removed in earlier batches.
- [x] **`extraction_bisimulation` does not reuse `extraction_possible`.** Independence is mathematically necessary (reachability refinement vs non-controllability refinement). Documented in decision log and paper remarks. (ExtractionPossibility.lean)
- [x] **Docstring line references to main.tex are brittle.** Fixed: replaced stale line numbers with section names in CoRefinementConvergence.lean:98-99. (CoRefinementConvergence.lean)
