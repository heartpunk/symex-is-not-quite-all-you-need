...
In plain terms: You can automatically derive executable formal specs from implementations---no one writes specs, everyone writes implementations, but you can't prove things about implementations directly. This technique lets you get the spec from the implementation (given its grammar). The main results are mechanized in Lean 4 with no admitted axioms.
\end{abstract}
...
\item $\mathcal{C}$ is a covering set of programs that enumerate productions in the chosen fragment grammar $\Gamma$, and alternatives in each such production, in a minimized way to be elaborated upon later
\item $\tau \in \mathcal{T}$ is a trace, or a sequence of states and labels for the transitions between them, generated when we run the host machine $H$ over the programming language implementation $\mathcal{I}$ and a program from $\mathcal{C}$, which implies all values relevant to updates. For this formulation, we assume these update-relevant values are available.
\item $\mathcal{T}$ is the set of all traces $\tau$
\item The semantics of any production decomposes into a sequence of deterministic steps between its holes. A \textbf{Hole-to-Hole (HTH) block} is one such step: given $h_i, h_j \in \text{holes}(\gamma)$ consecutive in evaluation order, the HTH block $\ell = (\gamma, h_i, \gamma, h_j)$ is the unique non-branching execution segment from $h_i$'s evaluation to $h_j$'s. These are the units over which we extract relational transformers $R_\ell$; their lack of internal branching makes symbolic execution tractable.
...
\item $\text{Alt}(s)$ is the set of \emph{all} feasible branch outcomes from state $s$ that are reachable by modifying $\pi(s)$. This is the completeness constraint: if outcome $o$ is achievable by changing $X$-state at $s$, then $o \in \text{Alt}(s)$. Branches depending on state outside $X$ are either implementation-internal (irrelevant to $G'$) or trigger $\pi$-refinement to include that state. Equivalently, $\text{Alt}(s)$ is the set of branch outcomes controllable by variations within the current projection fiber $\pi^{-1}(\pi(s))$; outcomes not controllable in that fiber trigger projection refinement.
...
\begin{enumerate}
\item \textbf{Grammar oracle} ($\Gamma$): Provides syntactic coverage of the chosen language fragment. This may be a known grammar or one inferred via grammar-mining techniques~\cite{bettscheider2024lookma}. The grammar oracle delimits the syntax space but is not a source of semantic uncertainty.
...
\begin{itemize}
\item $\mathcal{C}$ be a covering set of programs that exercises the productions in $\Gamma$
...
\end{enumerate}
Since $\text{observe}(\sigma, d) = \text{observe}(\sigma_2, d)$ for all $d$, observation faithfulness ($h_\text{faithful}$, using $\sigma \in \text{Reach}(H_\mathcal{I})$) gives $\sigma_2 = \sigma$. But $\sigma$ can take $\ell$, so $\sigma_2$ can too---contradiction. (Mechanized: \texttt{extraction\_possible} in \texttt{ExtractionPossibility.lean}.)
...
In the ICTAC setting ($\pi = \text{id}$)~\cite{lucanu2017generic}, bisimulation follows directly from trace correspondence without the reachability refinement machinery (\texttt{bisimulation\_of\_TraceCorrespondence\_id}).
...
\draw[decorate,decoration={brace,amplitude=10pt,raise=6pt}]
    (e1.east) -- (e2.west) node[midway,above=18pt,font=\small] {HTH block $\ell_1 = (\gamma, h_1, \gamma, h_2)$};
...
Each HTH block is identified by a label $\ell = (\gamma_s, h_i, \gamma_d, h_j)$ where:
...
This 4-tuple uniquely identifies each HTH block in the language. In the common case $\gamma_s = \gamma_d$ (intra-production). Different evaluation orders within a production yield different HTH blocks.
