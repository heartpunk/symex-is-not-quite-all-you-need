...
\item \textcolor{red}{An observation function $\text{observe} : \text{HostState} \to \text{Dim} \to \text{Value}$ over a finite dimension set}
...
\textcolor{red}{\textbf{On oracle instantiation.} None of the hypotheses are counterfactual. The \emph{grammar} is either given (language specification) or inferred via grammar-mining techniques. The \emph{branching oracle} is instantiated by selective symbolic execution engines (S2E, angr) and formal ISA semantics (Sail for ARM/RISC-V). The \emph{value-transformation oracle} is similarly instantiated by symbolic execution's production of path conditions and symbolic stores. \emph{Observation faithfulness} ($h_\text{faithful}$) requires only that \emph{reachable} states be distinguishable by observation---unreachable states need not be distinguished. This holds whenever the observation function includes all ISA-documented state (registers, memory, flags) that a program can reach.}
...
\textcolor{red}{\textbf{On bisimulation.} With the additional hypothesis that symex is \emph{exact} (biconditional with $H_\mathcal{I}.\text{step}$, adding $h_\text{symex\_complete}$), the extraction pipeline yields bisimulation among reachable states: both simulation directions $G' \preceq H_\mathcal{I}$ and $H_\mathcal{I} \preceq G'$ hold with witness relations restricted to $\text{Reach}(H_\mathcal{I})$. The proof constructs a strengthened fixpoint that additionally resolves observation disagreements among reachable states. At this fixpoint, $\pi$ is injective on reachable states: any two reachable $\sigma_1, \sigma_2$ with $\pi(\sigma_1) = \pi(\sigma_2)$ agree on all dimensions, so $h_\text{faithful}$ gives $\sigma_1 = \sigma_2$. Oracle completeness follows: the existential witness $\sigma_0$ in $R(\ell, \pi(\sigma), x')$ satisfies $\sigma_0 = \sigma$ by injectivity, so its step is the query state's step. No observation-determinism or functional-substitution hypothesis is needed. In the ICTAC setting ($\pi = \text{id}$), bisimulation follows directly from trace correspondence. This reachability restriction is natural: it excludes only states unreachable by any program execution, which are irrelevant to language semantics. For HTH fragments---straight-line regions between sentinel boundaries with bounded control flow by construction---symex completeness is typically achievable, making bisimulation the expected outcome in practice rather than a theoretical curiosity. (Mechanized: \texttt{extraction\_bisimulation} and \texttt{bisimulation\_of\_TraceCorrespondence\_id}.)}
