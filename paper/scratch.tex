...
\item \textcolor{red}{$O(s)$ is the branching oracle: given $s \in \text{BStates}(\mathcal{I})$, it produces constraints at the $\Sigma$ level that achieve each alternative in $\text{Alt}(s)$, which are then lifted to constraints over $\pi(s)$}
...
\textcolor{red}{Throughout this paper, the ``branching oracle $O$'' refers to this combined realization---selective symbolic execution for path feasibility plus formal ISA semantics (e.g., the K framework~\cite{rosu2010kframework}) for reachability---not an additional assumption beyond symex. We use $O$ as notational shorthand.}
...
What we need for value transformation inference is provided fully by symbolic execution engines. \textcolor{red}{When symbolically executing a basic block, the engine produces symbolic expressions showing how input values map to output values---this directly gives us the $\text{Update}(x, x')$ component of each $R_\ell$~\cite{voogd2025compositional,loow2025compositional}.}
...
\paragraph{Completeness.} \textcolor{red}{Given a complete branching oracle, this follows from the fixpoint construction.} Since by \textcolor{red}{assumption} the branching oracle can detect all branching behavior at the level of $H$, were there to be any state reachable in $H_\mathcal{I}$ that we did not find at least one exemplar trace for, then it's a contradiction with the \textcolor{red}{completeness} assumption. \textcolor{red}{This is conditional on oracle completeness (symex being biconditional with $H_\mathcal{I}.\text{step}$); given this, the bisimulation Corollary (Section~V-E, \texttt{extraction\_bisimulation}) formally establishes both simulation directions, ensuring no reachable behavior is missed.}
...
\begin{enumerate}
\item \textcolor{red}{\textbf{Grammar oracle} ($\Gamma$): Provides full syntactic coverage of the language fragment. This may be a known grammar or one inferred via grammar-mining techniques~\cite{bettscheider2024lookma}. The grammar oracle delimits the syntax space but is not a source of semantic uncertainty.}
...
\textcolor{red}{\textbf{On generality.}
