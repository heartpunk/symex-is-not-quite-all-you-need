...
\item \textcolor{red}{\textbf{Observation faithfulness} ($h_\text{faithful}$): $\forall \sigma_1, \sigma_2.\; (\forall d.\; \text{observe}(\sigma_1, d) = \text{observe}(\sigma_2, d)) \implies \sigma_1 = \sigma_2$. The observation function is injective: distinct host states are distinguishable by some dimension. This is the key hypothesis enabling co-refinement convergence (Section~V).}
...
\item \textcolor{red}{The observation function is faithful ($h_\text{faithful}$): distinct host states are distinguishable by some dimension}
\end{enumerate}
\textcolor{red}{then there exist $\pi$ and $R$ satisfying the co-refinement fixpoint condition, and hence $G' \preceq H_\mathcal{I}$.}
