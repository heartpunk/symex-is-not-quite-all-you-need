...
\begin{itemize}
\item \textcolor{red}{\textbf{Projection.} For a tracked dimension set $X \subseteq \text{Dim}$, define $\pi_X(\sigma)(d) = \text{observe}(\sigma, d)$ if $d \in X$, else a default value. Two states have the same projected configuration iff they agree on all tracked dimensions.}

\item \textcolor{red}{\textbf{Oracle.} For each label $\ell$, define $R_\ell^X(x, x') \iff \exists \sigma, \sigma'.\; \pi_X(\sigma) = x \land \sigma \xrightarrow{\ell} \sigma' \land \pi_X(\sigma') = x'$. This is sound by construction: every concrete transition witnesses the oracle claim.}
...
\textcolor{red}{\textbf{Fixpoint property.} At the fixpoint $X^* = X'$, no non-$X^*$-controllable transitions exist among reachable states. Suppose for contradiction that $\sigma \in \text{Reach}(H_\mathcal{I})$ can take transition $\ell$ but some $\sigma_2$ with $\pi_{X^*}(\sigma_2) = \pi_{X^*}(\sigma)$ cannot. Then for every dimension $d$: if $d \in X^*$, the projection equality gives $\text{observe}(\sigma, d) = \text{observe}(\sigma_2, d)$; if $d \notin X^*$, the fixpoint condition ensures no such divergent witness exists (otherwise $d$ would have been added). Thus $\text{observe}(\sigma, d) = \text{observe}(\sigma_2, d)$ for all $d$, and by $h_\text{faithful}$, $\sigma_2 = \sigma$---contradicting the assumption that $\sigma_2$ cannot take $\ell$.}
...
\begin{itemize}
\item \textcolor{red}{\emph{Oracle soundness} holds by construction of $R^{X^*}$: every concrete transition $\sigma \xrightarrow{\ell} \sigma'$ provides its own existential witness.}
...
\textcolor{red}{\textbf{On oracle instantiation.} None of the hypotheses are counterfactual. The \emph{grammar} is either given (language specification) or inferred via grammar-mining techniques. The \emph{branching oracle} is instantiated by selective symbolic execution engines (S2E, angr) and formal ISA semantics (Sail for ARM/RISC-V). The \emph{value-transformation oracle} is similarly instantiated by symbolic execution's production of path conditions and symbolic stores. \emph{Observation faithfulness} ($h_\text{faithful}$) holds whenever the observation function includes all ISA-documented state (registers, memory, flags)---distinct machine states are distinguishable by some documented dimension.}
