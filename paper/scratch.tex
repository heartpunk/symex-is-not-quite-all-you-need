...
The main theorem of this paper is proved for the specific setting of LTS extraction from an ISA implementation. However, the core argument depends on surprisingly little structure. We mechanize (\texttt{Learnability.lean}, independent of all other project modules) five sufficient \emph{learnability preconditions} under which \emph{any} system expressible as $\text{State} \to \text{Label} \to \text{State} \to \text{Prop}$ admits a faithful projected model via iterative refinement:}

\textcolor{red}{\begin{enumerate}
\item \textbf{Finiteness}: the observation space (dimensions) is finite.
\item \textbf{Enumerability}: dimensions can be iterated. In the LTS case, grammar conformance provides this---one template per grammatical rule.
\item \textbf{Identifiability}: observations distinguish relevant states. Formally, for any relevant $s_1$ and arbitrary $s_2$, if $s_1$ and $s_2$ agree on all observations then $s_1 = s_2$. Note that $s_2$ need not be relevant---this asymmetry is load-bearing in the controllability argument.
\item \textbf{Separability}: (proved, not assumed) at the refinement fixpoint, the projection captures all relevant distinctions.
\item \textbf{Extractibility}: a sound oracle witnesses behavior---every real behavior has an oracle witness.
\end{enumerate}}

\textcolor{red}{Under these conditions, \texttt{extraction\_exists} proves the existence of a dimension set $X^*$ at which the projected oracle is sound for all relevant behaviors and every relevant state's behavior availability is controllable (same projection implies same behavioral capabilities). With a complete oracle, \texttt{exact\_extraction} strengthens this to injectivity on relevant states, yielding the full bisimulation. The proof strategy---contradiction plus pigeonhole on a monotone chain in $\text{Finset}(\text{Dim})$---is the same one used in the main paper, abstracted away from LTS-specific vocabulary.}

\textcolor{red}{The relation $\text{State} \to \text{Label} \to \text{State} \to \text{Prop}$ is general enough to encode type systems (typing judgments as transitions), parsers (parse steps), effect systems, and other semantic domains---this paper validates the framework for the LTS case only; the expressiveness of the abstract skeleton is a separate claim from what we instantiate here. Instantiating the preconditions---particularly faithfulness with unconstrained $s_2$ and constructing a sound oracle---for non-LTS domains is non-trivial. We attempted to establish useful cost bounds for the general class of extraction algorithms these preconditions admit, but the formal content reduces to the iteration bound ($n \leq |\text{Dim}|$); the tractability of specific instantiations beyond the LTS case remains future work.}
