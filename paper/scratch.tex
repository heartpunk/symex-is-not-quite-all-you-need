...
\textcolor{red}{The relation $\text{State} \to \text{Label} \to \text{State} \to \text{Prop}$ is general enough to encode type systems (typing judgments as transitions), parsers (parse steps), effect systems, and other semantic domains, though the encoding for non-LTS domains may be non-trivial. We attempted to establish useful cost bounds for the general class of extraction algorithms these preconditions admit, but the formal content reduces to the iteration bound ($n \leq |\text{Dim}|$); the tractability of specific instantiations beyond the LTS case remains future work.}
