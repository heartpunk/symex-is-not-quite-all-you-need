...
\item \textcolor{red}{Observation faithfulness ($h_\text{faithful}$): $\forall \sigma_1, \sigma_2.\; (\forall d.\; \text{observe}(\sigma_1, d) = \text{observe}(\sigma_2, d)) \to \sigma_1 = \sigma_2$}
\end{enumerate}
\textcolor{red}{there exist $\pi$ and $R$ satisfying the co-refinement fixpoint condition, and hence $G' \preceq H_\mathcal{I}$.}

\textcolor{red}{\emph{Note.} The testing infrastructure---covering sets, reachability oracle, label determinism---is used by the differential causality analysis (Section~IV-B) to \emph{discover} which dimensions matter, but the convergence proof itself requires only the three hypotheses above.}
